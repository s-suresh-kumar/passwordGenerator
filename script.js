...
function generatePassword() {
  ...
  const lc = confirm(
    "Do you want to include lower case characters in password"
  );
  const uc = confirm(
    "Do you want to include upper case characaters in password?"
  );
...
*/
      ...
      while (userContinue && inputError) {
        ...
        if (wantToContinue === false) {
          alert(
            "No passord will be generated; valid psw length needed (8 - 128 chars)"
          );
          userContinue = false;
        } else {
          pswLen = prompt("Provide password length 8 - 128 chars");
  ...
  if (!userContinue) {
    ...
    if (!(lc || uc || num || sp)) {
      errorText +=
        ", Atleast one char type should be included for Password Generation.";
